...
\#### \[x] S35 — Controle protético  ·  \*DIVIDIDA: \[x] S35a (backend: ProstheticOrder + serviço + atraso) · \[x] S35b (tela web)\*
...
\#### \[x] S50 — IA: agente de agendamento (WhatsApp)
...
\> \*\*2026-07-27:\* código 100% escrito e mergeado (módulo + 3 ambientes + 2 Dockerfiles), validado com `terraform validate`/`fmt` + `docker build`/`docker run` reais — mas SEM conta DigitalOcean própria ainda, então `terraform plan` nunca rodou contra a API real. Checkbox fica `\[ ]` até isso acontecer. Ver log da sessão.\*
...
\- \*\*2026-06-23 · S35b — Controle protético: tela web (FECHA a S35)\*\*

  \- \*O que foi feito:\* UI do controle de laboratório. \*\*api-client:\* +`listProstheticOrders(status?)`/`createProstheticOrder(input)`/`sendProstheticOrder(id,date?)`/`receiveProstheticOrder(id)`/`cancelProstheticOrder(id)` + tipo `ProstheticOrder` (com `isLate`).\* Página \*\*`/protese`\*\* (Server Component, lista pedidos + pacientes) + `prosthetic-list.tsx` (client): form \*\*"Novo pedido"\*\* (paciente + laboratório + descrição + prazo → Registrar); lista de pedidos com \*\*pedidos atrasados destacados (borda vermelha + badge "Atrasado")\*\*, badge de status (Solicitado/No laboratório/Recebido/Cancelado), prazo + data de recebimento; ações por status: \*\*"Enviar ao lab"\*\* (com input de prazo), \*\*"Marcar recebido"\*\*, "Cancelar". Server Actions BFF re-listam após cada mutação.

  \- \*Arquivos tocados:\* `packages/api-client/src/index.ts` (5 métodos + 1 tipo), `apps/web/app/protese/{page.tsx,actions.ts,prosthetic-list.tsx}` (novos). Sem backend novo (consome S35a), sem migration.

  \- \*Decisões:\* o flag `isLate` vem calculado do backend (a UI só destaca — borda vermelha + badge). Ações condicionadas ao status (REQUESTED→Enviar/Cancelar; SENT→Receber/Cancelar; RECEIVED→sem ação). Página standalone `/protese` (navegação comum entre as telas web segue pendente).
...
\- \*\*2026-06-26 · S47 — Royalties (FECHA a S47)\*\*

  \- \*O que foi feito:\* royalties por unidade de franquia. Schema: \*\*`Royalty`\*\* (tenantId, unitId, periodStart/End, baseCents, percent, amountCents, status `RoyaltyStatus` PENDING/CHARGED/PAID/CANCELLED, asaasPaymentId/pixPayload/boletoBarcode, paidAt; `@@unique([tenantId, unitId, periodStart, periodEnd])` = idempotência) + migration aditiva. `RoyaltyService` (no módulo `network`): `calculate` (anti-IDOR: valida que a unidade pertence ao tenant via `unit.findFirst`→ForbiddenException; valida periodStart<periodEnd; agrega `Charge.totalCents` não-canceladas da unidade no período; calcula `amountCents=round(base*percent/100)` SERVER-SIDE; P2002→ConflictException para dupla do mesmo período), `list` (filtro opcional por unidade), `charge` (PENDING→CHARGED, stub ou Asaas real; ConflictException se não PENDING), `markPaid` (→PAID+paidAt; ConflictException se PAID/CANCELLED), `cancel` (→CANCELLED; ConflictException se PAID/CANCELLED). `RoyaltyController` em `POST /network/royalties` (billing:write), `GET /network/royalties` (billing:read), `POST /network/royalties/:id/charge|pay|cancel` (billing:write). `AsaasService` provido no `AccessModule` (stateless, anti-SSRF, fail-closed). \*\*6 testes unitários.\*\* Sem UI web (backend-only, conforme spec).

  \- \*Arquivos tocados:\* `apps/api/prisma/schema.prisma` (+`Royalty` +`RoyaltyStatus` +back-relations Tenant/Unit), `apps/api/prisma/migrations/*_s47_royalty/` (aditiva), `apps/api/src/network/{royalty.dto.ts,royalty.service.ts,royalty.controller.ts}` (novos), `apps/api/src/network/access.module.ts` (+RoyaltyController/RoyaltyService/AsaasService), `apps/api/test/royalty.service.spec.ts` (6 casos). Sem dep nova. \*Permissions `billing:*` reusadas — sem re-seed.\*

  \- \*Decisões:\* cobrança **síncrona** (não em fila BullMQ) — royalty é ação admin deliberada, não fluxo de alto volume (§8 simplicidade). Stub quando Asaas não configurado (mesmo padrão S12/S19/S25). Sem UI web — a spec não lista sub-sessão "b"; tela pode entrar futuramente junto da S48. \*\*Bugs corrigidos durante a sessão:\*\* (1) `Principal` importado de `permissions.guard` (não exportado) — corrigido p/ `jwt.strategy`; (2) `TenantScope` importado mas não usado (anti-IDOR via Prisma direto) — removido; (3) `actorId` no `charge()` não utilizado — removido da assinatura e do controller.

  \- \*Verificação:\* `pnpm build`/`lint` (8/8)/`test` (243 pass, 2 skip; +6)/`format:check`/`audit` (0 high/critical; 1 moderate dev-only) \*\*verdes\*\*. \*\*AO VIVO\*\* (PG+Redis efêmeros 5460/6400, stub Asaas 9090): cobrança R\$900 semeada → `calculate` → \*\*base 90000, amount 9000 (10%), PENDING\*\* → dupla → \*\*409 ✓\*\* → list count 1 → `charge` → \*\*CHARGED, asaasId=asaas-stub-roy1\*\* → re-charge → \*\*409 ✓\*\* → `markPaid` → \*\*PAID, paidAt set\*\* → anti-IDOR (unidade de outro tenant) → \*\*403 ✓\*\* → DB: status PAID, baseCents 90000, amountCents 9000, paidAt true. Containers efêmeros derrubados; script descartável removido; árvore limpa (só os 6 arquivos da S47).

  \- \*Pendências p/ próxima:\* \*\*S47 COMPLETA.\*\* Próxima no backlog: \*\*S48 — Central de agendamentos\*\* (`network/scheduling-center.service.ts`, visão consolidada de múltiplas unidades, roteamento de marcação). Depende de S45+S6. Herdadas: UI web de royalties, filtro de período na tela de unidades, retrofit de filtro por unidade nos módulos, navegação comum no web, `convertedPatientId` no fechamento do lead, `Professional`/`Room` dedicados, extrair `@vero/mobile-shared`, as de sempre.

\- \*\*2026-06-26 · S48 — Central de agendamentos (FECHA a S48)\*\*

  \- \*O que foi feito:\* operador agenda e visualiza em todas as unidades autorizadas a partir de um único ponto, sem precisar trocar a unidade ativa na sessão. `SchedulingCenterService` (módulo `network`): `getConsolidated(tenantId, userId, query)` — resolve os `unitId` autorizados via `UserUnit` do usuário; se `query.unitId` informado e não autorizado devolve \*\*lista vazia\*\* (não 403 — a autorização geral está ok, só o filtro não casa); busca agendamentos com join enriquecido `CENTRAL_SELECT` (unit.name / professional.name / patient.name+phone) ordenado por unitId→startsAt, limite 500. `book(tenantId, userId, dto)` — valida `userUnit.findFirst({ where: { tenantId, userId, unitId, unit: { deletedAt:null } } })` → \*\*ForbiddenException se null (anti-IDOR)\*\*; delega p/ `AppointmentService.create` (re-checa conflito de horário → 409). `listAuthorizedUnits` — unidades autorizadas p/ o seletor do form. `SchedulingCenterController` (Permissions `appointment:read|write` — apenas equipe; pacientes barrados): `GET /network/scheduling`, `GET /network/scheduling/units`, `POST /network/scheduling/appointments`. `CentralQueryDto` (from/to/unitId opcionais). \*\*Web:\* página `/central` (Server Component, force-dynamic): busca units + appointments + professionals + patients em `Promise.allSettled` (fail-soft); componente client `SchedulingCenter`: form de novo agendamento (selects unidade/profissional/paciente + datetime-local início/fim + observação → `bookCentralAction` → Server Action BFF); \*\*tabela consolidada\*\* com colunas Unidade/Início/Fim/Paciente(nome+telefone)/Profissional/Status (badges coloridos). api-client: +`centralSchedulingUnits()`/`centralAppointments(params?)`/`bookCentralAppointment(input)` + tipos `CentralUnitOption`/`CentralAppointment`. 5 testes unitários.

  \- \*Arquivos tocados:\* `apps/api/src/network/{scheduling-center.service,scheduling-center.controller}.ts` + `dto/scheduling.dto.ts` (novos), `apps/api/src/network/access.module.ts` (+AppointmentModule + SchedulingCenter*/Service), `apps/api/test/scheduling-center.service.spec.ts` (5 casos), `packages/api-client/src/index.ts` (+3 métodos +2 tipos), `apps/web/app/central/{page,actions,scheduling-center}.tsx` (novos). Sem migration, sem dep nova.

  \- \*Decisões:\* `getConsolidated` com `unitId` não-autorizado → \*\*lista vazia (não 403)\*\* — o operador tem acesso global, só o filtro não bate; o aceite diz "operador agenda em qualquer unidade autorizada", não que filtrar por não-autorizada é proibição. Reusei `AppointmentService.create` (não duplica lógica de conflito/disponibilidade). `CENTRAL_SELECT` como const de módulo (fonte única do select + tipo `CentralAppointment` do api-client). \*\*BUG crítico corrigido durante a sessão:\*\* `import type { CreateAppointmentDto }` (type-only import) remove a classe em tempo de compilação → em runtime o NestJS não consegue resolver os metadados do `class-transformer`/`class-validator`; o `ValidationPipe` global com `whitelist:true` rejeita TODAS as propriedades do body com "should not exist" (400). Corrigido p/ `import { CreateAppointmentDto }` (import de valor). \*\*Lição:\* decorators de parâmetro e `ValidationPipe` exigem que a classe DTO esteja disponível em runtime — nunca usar `import type` p/ DTOs usados em `@Body()`.\*

  \- \*Verificação:\* `pnpm lint` (8/8)/`test` (248 pass, 2 skip; +5)/`build` (4/4, `/central` compilou dinâmico)/`format:check`/`audit` (0 high/critical; 1 moderate dev-only) + `tsc -p tsconfig.build.json --noEmit` \*\*todos verdes\*\*. \*\*AO VIVO\*\* (PG+Redis efêmeros 5461/6401, API HTTP real): (1) login GESTOR → token ✓; (2) `GET /network/scheduling/units` → Filial + Matriz ✓; (3) `GET /network/scheduling` sem agendamentos → count 0 ✓; (4) `POST /network/scheduling/appointments` (Matriz) → 201, join completo `unit.name`+`professional.name`+`patient.name+phone` ✓; (5) `GET` após criar → count 1 ✓; (6) `POST` p/ unidade \*\*não autorizada → 403 (anti-IDOR)\*\* ✓; (7) token de paciente em `/units` → 403 ✓; (8) `GET?unitId=matriz` → 1 agendamento filtrado ✓; (9) `GET?unitId=nao-autorizado` → 0 (lista vazia, não 403) ✓. 9/9 cenários do aceite. Containers efêmeros derrubados; arquivos descartáveis removidos; árvore limpa (só os 7 arquivos da S48). \*Nota:\* JSON parseado via Python3 (`json.load`/`json.loads`) — `jq` não disponível no ambiente.

  \- \*Pendências p/ próxima:\* \*\*S48 COMPLETA.\*\* Próximas no backlog: \*\*S49 — API pública + white-label\*\* e \*\*S50–S51 (IA: agente de agendamento + insights)\*\*. Herdadas: UI web de royalties, filtro de período na tela `/unidades`, retrofit de filtro por unidade nos módulos (agenda first, `@ActiveUnitId` já pronto), navegação comum no web (menu entre as páginas), `convertedPatientId` no fechamento do lead CRM, modelos `Professional`/`Room` dedicados, extrair `@vero/mobile-shared` (muito duplicado entre os 2 apps), device-build dos apps (usuário), `docker-compose.yml`, `start`→`dist/src/main.js` mismatch, lockout por conta, invalidar cache `rbac:perms` no PERMISSION_CHANGED, `MessageLog` no banco.

\- \*\*2026-07-25 · S49 — API pública + white-label (FECHA a S49)\*\*  ·  \*retomada de WIP não-commitado de sessão anterior (schema+service+controller+teste já existiam sem log nem revisão) — esta sessão revisou, corrigiu 2 falhas de segurança, ligou o disparo real de evento, construiu a tela web e fechou a documentação\*

  \- \*O que foi feito:\* API pública v1 para terceiros + gestão white-label. \*\*Backend (já existente, revisado):\* `ApiKey` (hash SHA-256 da chave, prefixo p/ exibição, escopos, `rateLimit`, expiração) + `OutboundWebhook` + `Tenant.brandColor/logoUrl`. `ApiKeyGuard` (`@Public`+`@UseGuards`): valida a chave, aplica rate limit por chave (Redis INCR/EXPIRE janela 60s) e escopos (`@ApiKeyScopes`). `ManageController` (`/manage/*`, JWT+`billing:read|write`, equipe): CRUD de chaves (plain retornado 1x), branding, CRUD de webhooks (secret retornado 1x). `V1Controller` (`/api/v1/*`, `@Public`+API key): `GET appointments|patients` (escopados), `GET branding` (qualquer chave válida).\* \*\*2 falhas corrigidas nesta sessão (ver Decisões).\* \*\*Disparo real de evento (faltava):\* `AppointmentService.create` agora chama `ApiKeyService.deliverEvent(tenantId, "appointment.created", {...})` fire-and-forget após persistir — sem isso a "entrega de webhook" do aceite era mecanismo morto, nunca acionado por nenhuma ação de domínio.\* \*\*Web (nova):\* página `/api-publica` — 3 seções: Chaves de API (criar com nome+escopos+rate limit, revelar o plain 1x, listar, revogar), Marca (color picker + URL do logo, preview), Webhooks (criar com URL+eventos, revelar o secret 1x, listar, revogar). api-client já tinha os métodos de gestão + `createV1Client` (cliente p/ terceiros, Bearer da API key) do WIP anterior.

  \- \*\*\*2 falhas de segurança corrigidas (achadas na revisão do WIP, antes de qualquer commit):\*\*\* \*\*(1) Assinatura HMAC do webhook nunca verificaria do lado do terceiro\*\* — o código original assinava com `secretHash` (SHA-256 do segredo, guardado pra nunca ser reversível, como uma senha) mas o terceiro só recebe o \*\*secret PLAIN\*\* (1x, na criação); ele computaria `HMAC(secret_plain, body)` p/ verificar, e o servidor mandava `HMAC(sha256(secret), body)` — nunca bateria, a assinatura seria inverificável em qualquer integração real. \*\*Corrigido:\* campo renomeado `secretEnc` — o segredo é CIFRADO em repouso (AES-256-GCM via `CryptoService`, mesmo padrão da S26/S28), \*\*decifrado\*\* no momento de cada entrega e usado como chave HMAC (reversível, ao contrário de um hash). Provado por teste: reproduzi a assinatura com o secret plain do lado "terceiro" e bateu com o header enviado pelo servidor — e provado \*\*AO VIVO\*\*: um receiver HTTP real recebeu o webhook dum `POST /appointments` de verdade e a assinatura bateu.\* \*\*(2) Sem anti-SSRF na URL do webhook\*\* — `CreateWebhookDto.url` aceitava QUALQUER URL e `deliverEvent` fazia `fetch` direto nela; um tenant malicioso/comprometido (role `billing:write`) podia registrar um webhook apontando pro `169.254.169.254` (metadados de nuvem) ou `localhost:6379` (Redis interno) pra fazer o backend do Vero atacar a própria infra — exatamente o padrão de risco que o CLAUDE.md §4 já exige mitigar em toda integração externa (WhatsApp/Asaas/NFS-e/SPC já bloqueiam). \*\*Corrigido:\* mesmo helper `isInternalHost` (loopback/RFC1918/169.254/IPv6 local) replicado no `ApiKeyService`, checado em `createWebhook` (400 se host interno em produção) E de novo em `deliverEvent` (pula/loga, não derruba os outros webhooks).\*\*

  \- \*Arquivos tocados:\* `apps/api/prisma/schema.prisma` (`secretHash`→`secretEnc` +doc), `apps/api/prisma/migrations/20260626234716_s49_api_key/migration.sql` (ajustada — nunca havia sido aplicada em nenhum banco, então editada em vez de nova migration), `apps/api/src/public-api/{api-key.service,api-key.guard,public-api.controller,public-api.module,dto/api-key.dto}.ts`, `apps/api/src/appointment/{appointment.service,appointment.module}.ts` (dispara o evento), `apps/api/test/{api-key.service,appointment.service}.spec.ts` (+7 casos novos: assinatura verificável, anti-SSRF create+deliver), `packages/api-client/src/index.ts` (métodos/tipos já existiam do WIP), `apps/web/app/api-publica/{page,actions,public-api-view}.tsx` (novos). \*\*Fora do escopo nominal da S49, achado durante o DoD:\* `pnpm audit` acusou 16 HIGH novos (drift de 1 mês desde a S48 — nenhuma dependência tinha sido tocada nesta sessão) — \*\*Next.js 15.5.19→15.5.22\*\* (corrige 3 HIGH: DoS em Server Actions + \*\*2 SSRF\*\* em Server Actions/rewrites — relevante pro mesmo tema de segurança desta sessão), + overrides novos em `package.json` (`sharp`, `brace-expansion` ×3 faixas, `js-yaml` blanket, `shell-quote`, `fast-uri`, `postcss` atualizado p/ ≥8.5.18, `body-parser`) — todos transitivos de tooling de build/teste (nestjs/cli, jest, expo/cli) exceto Next e body-parser (produção). `pnpm audit` final: \*\*zero vulnerabilidades\*\* (era "0 high/critical" há 1 mês; ficou assim de novo).\*

  \- \*Decisões:\* endpoints de gestão (`/manage/*`) reusam `billing:*` (sem nova permission/re-seed). `V1Controller` é `@Public`+`ApiKeyGuard` própria (não a cadeia JWT) — mesmo padrão do webhook Asaas (S20)/Evolution (S12). Único evento de domínio ligado por ora: `appointment.created` (a UI só oferece esse na lista de eventos — não inventei uma taxonomia maior sem consumidor real). Escopos oferecidos na UI: só os 2 que o V1Controller realmente usa (`appointments:read`, `patients:read`) — mesmo raciocínio. Anti-SSRF só ativo em produção (`NODE_ENV`) — mesma convenção das demais integrações, permite testar contra `localhost` em dev. `CryptoService` provido \*\*stateless\*\* no `PublicApiModule` (não importa o `RecordModule` inteiro) — mesmo padrão da S28/S29.

  \- \*Verificação:\* `pnpm lint` (8/8)/`tsc --noEmit` da api/`test` (260 pass, 2 skip; +7)/`build` (4/4, `/api-publica` compilou dinâmico, sem cache)/`format:check` (limpo nos arquivos tocados; 3 arquivos de `apps/web/app/central` seguem com drift de formatação \*pré-existente da S48\*, não tocados aqui)/`pnpm audit` \*\*zero vulnerabilidades\*\* — todos verdes. \*\*AO VIVO\*\* (PG+Redis efêmeros 5462/6402, API HTTP real + receiver HTTP real p/ o webhook): criar chave → plain `vero_*` retornado 1x → `GET /api/v1/appointments|patients` com a chave → 200 (escopos concedidos); chave sem `patients:read` → \*\*403\*\*; chave inválida → \*\*401\*\*; \*\*rate limit (5/min) excedido → 403\*\*; revogar → \*\*204\*\* e uso subsequente → \*\*401\*\*; branding: `PATCH` grava, `GET /api/v1/branding` reflete via QUALQUER chave válida; webhook: criar → secret 1x → \*\*`POST /appointments` real dispara o webhook\*\* → receiver recebeu `X-Vero-Event: appointment.created` + `X-Vero-Signature` cuja assinatura, recomputada com o secret plain do lado do "terceiro", \*\*bateu exatamente\*\* (prova do bugfix). \*\*Web (Playwright):\* login → `/api-publica` → criar chave (revela "Copie a chave agora") → salvar marca (cor+logo) → criar webhook (revela "Copie o segredo agora") → screenshot confirma as 3 seções, chaves/webhooks anteriores listados. Ambiente efêmero derrubado (containers + `.env` removidos); scripts descartáveis removidos; árvore limpa.

  \- \*Pendências p/ próxima:\* \*\*S49 COMPLETA — FECHA a FASE 5 planejada até aqui (S45-S49; S46/S47/S48 já fechadas).\*\* Próxima no backlog: \*\*S50 — IA: agente de agendamento (WhatsApp)\*\* (depende de S12+S15) ou \*\*S51 — IA: insights\*\* (depende de S42+S44). Herdadas: mais eventos de domínio no webhook (hoje só `appointment.created` — budget aprovado/pagamento recebido são candidatos óbvios), ler estado de `isActive`/uso da chave num painel de auditoria, drift de formatação em `apps/web/app/central` (S48, não tocado), UI web de royalties, filtro de período em `/unidades`, retrofit de filtro por unidade nos módulos, navegação comum no web, `convertedPatientId` no CRM, modelos `Professional`/`Room` dedicados, extrair `@vero/mobile-shared`, device-build dos apps (usuário), `docker-compose.yml`, `start`→`dist/src/main.js`, lockout por conta, invalidar cache `rbac:perms`, `MessageLog` no banco.

\- \*\*2026-07-26 · S50 — IA: agente de agendamento via WhatsApp (FECHA a S50)\*\*  ·  \*decisão do usuário: motor real de IA (Claude/Anthropic), não parser determinístico\*

  \- \*O que foi feito:\* agente conversacional que agenda/confirma consultas por WhatsApp respeitando os guardrails do §5. \*\*`AnthropicService`\*\* (`integrations/anthropic/`) — proxy fino da Claude Messages API (endpoint FIXO `api.anthropic.com`, sem input do usuário → sem SSRF; fail-closed sem `ANTHROPIC_API_KEY`; suporta tool-use). \*\*`AgentService`\*\* (`ai/agent.service.ts`) — núcleo do agente: resolve tenant/paciente pelo TELEFONE (mesmo padrão anti-IDOR do webhook S12b; \*\*só atende paciente já cadastrado\*\* — lead novo segue pelo booking público S15b/CRM), mantém histórico de conversa em Redis (TTL 30min, capado a 20 mensagens), chama a Claude com 4 ferramentas (`list_professionals`, `find_slots`, `book_appointment`, `confirm_appointment`) num loop capado em \*\*4 rounds\*\* (guardrail anti-loop-infinito). \*\*O ponto central do §5 ("nunca decide sozinho, só propõe; backend valida"):\* a ferramenta `book_appointment` SEMPRE re-valida via `SlotService.openSlots` (mesma lógica do booking público S15a) e só então chama `AppointmentService.create` — que re-checa conflito/disponibilidade de verdade; se a IA "acha" que um horário está livre mas não está, a chamada retorna erro e a IA é obrigada a replanejar, nunca finge sucesso.\* \*\*Disclosure de IA (§5):\* HARD-CODED (não deixado ao modelo) — prefixado deterministicamente na primeira resposta de toda conversa nova, nunca nas seguintes.\* Integrado ao webhook existente: `WhatsAppWebhookService.handleInbound` mantém o caminho rápido determinístico da S12b (texto afirmativo + consulta SCHEDULED pendente → confirma direto, sem custo de IA) e delega ao agente QUALQUER outro caso (não-afirmativo, ou afirmativo sem consulta pendente) — só esse caminho estreito evita a IA.

  \- \*Arquivos tocados:\* `apps/api/src/integrations/anthropic/anthropic.service.ts` (novo), `apps/api/src/ai/{agent.service,ai.module}.ts` (novos), `apps/api/src/integrations/whatsapp/{whatsapp-webhook.service,whatsapp.module}.ts` (+AgentService/WhatsAppService injetados, +ramo do agente), `apps/api/src/app.module.ts` (+AiModule), `apps/api/src/config/env.validation.ts` + `.env.example` (+`ANTHROPIC_API_KEY`/`ANTHROPIC_MODEL`, opcionais), testes `apps/api/test/{agent.service,whatsapp-webhook.service}.spec.ts` (+7 novos, atualizado). Sem migration (não precisou de schema novo — reusa Patient/Appointment/Availability). \*\*Sem UI\*\* (a spec da S50 é só backend + orquestração via WhatsApp). \*Nota: o `book_appointment` do agente chama o MESMO `AppointmentService.create` que a S49 já instrumentou para disparar webhooks de saída em `appointment.created` — o agente ganha essa entrega de graça, sem precisar tocar `appointment.service.ts` de novo.\*

  \- \*Decisões:\* modelo default \*\*`claude-sonnet-5`\*\* (configurável via `ANTHROPIC_MODEL`) — a diretriz do ambiente é preferir o modelo mais capaz disponível; trocar para um mais barato (Haiku) fica a critério do usuário via env, sem mudar código. Ferramentas exportam só o que o agente PRECISA (4 tools) — nada de taxonomia especulativa. `confirm_appointment` resolve a PRÓXIMA consulta futura do paciente (sem exigir um id na conversa) e reusa a confirmação idempotente de `MeService` (S11/S12b) com `source: WHATSAPP` — mesma linha de auditoria/idempotência já existente. Conversão de fuso horário reusa `wallTimeToUtc` de `agenda.util.ts` (S15) em vez de reimplementar — "já existe algo similar?" aplicado. \*\*Escopo explicitamente FORA:\* resolução de tenant para um número de WhatsApp totalmente novo (sem paciente em nenhum tenant) — limitação pré-existente e já documentada desde a S12b ("mapa instance→tenant p/ multi-tenant real"); o agente responde com uma orientação a procurar a recepção nesse caso, sem tentar adivinhar o tenant.\*

  \- \*Verificação:\* `pnpm lint` (8/8)/`tsc --noEmit` da api/`test` (268 pass, 2 skip — `agent.service.spec.ts` novo com 7 casos + `whatsapp-webhook.service.spec.ts` reescrito p/ cobrir o ramo do agente)/`build` (4/4, sem cache)/`format:check` (limpo nos arquivos tocados; os 3 de `apps/web/app/central` seguem com drift pré-existente da S48, não tocados)/`pnpm audit` \*\*zero vulnerabilidades\*\* (sem dep nova) — todos verdes. \*\*AO VIVO\*\* (PG+Redis efêmeros 5463/6403, \*\*contexto Nest real\*\* via `createApplicationContext`, `global.fetch` interceptado para o endpoint fixo da Anthropic + a base do Evolution — mesma técnica da S12a/S13b para integrações de host fixo): paciente novo com telefone conhecido + disponibilidade semeada para amanhã → mensagem \*\*"Oi, quero marcar uma consulta"\*\* (sem consulta pendente, cai no agente) → stub simula Claude chamando \*\*list\_professionals → find\_slots → book\_appointment\*\* (4 rounds) → \*\*agendamento REAL persistido no banco\*\* (`SCHEDULED`, horário correto convertido do fuso America/Sao\_Paulo) → resposta enviada via WhatsApp \*\*com a disclosure\*\* (1ª mensagem da conversa). \*\*2ª mensagem\*\* na mesma conversa → resposta SEM disclosure (estado persistido no Redis, confirmado). Testes unitários cobrem adicionalmente: horário indisponível NÃO chama `AppointmentService.create` (o `tool_result` de erro devolvido à IA foi verificado via snapshot dos argumentos no momento da chamada — `history` é mutado depois, então guardar a referência crua daria falso positivo); cap de 4 rounds interrompe o loop se a IA insistir em ferramentas; telefone desconhecido nunca chega a gastar uma chamada à IA (guarda de custo).

  \- \*Pendências p/ próxima:\* \*\*S50 COMPLETA.\*\* Próxima no backlog: \*\*S51 — IA: insights\*\* (depende de S42+S44) — painel de sugestões a partir de dados agregados do tenant (conversão/inadimplência/ocupação), com disclosure de IA. Depois disso, \*\*Fase 6\*\* (S52-S57: CI/CD, infra Terraform/DigitalOcean, hardening, observabilidade, submissão às lojas) — é onde entraria um ambiente de produção de verdade. Herdadas: resolução de tenant p/ WhatsApp totalmente novo (multi-instância Evolution), mais ferramentas do agente (cancelar/reagendar), mais eventos de domínio no webhook público (S49), drift de formatação em `apps/web/app/central`, UI web de royalties, filtro de período em `/unidades`, navegação comum no web, modelos `Professional`/`Room` dedicados, extrair `@vero/mobile-shared`, device-build dos apps (usuário), `docker-compose.yml`, `start`→`dist/src/main.js`, lockout por conta, invalidar cache `rbac:perms`, `MessageLog` no banco.

\- \*\*2026-07-27 · S51 — IA: insights (FECHA a S51)\*\*  ·  \*retomada de WIP não-commitado de sessão anterior (backend+teste+tela web já existiam sem log nem revisão) — esta sessão revisou o código, corrigiu formatação, removeu os scripts de verificação descartáveis, rodou o Definition of Done completo e validou ao vivo com stack fresco\*

  \- \*O que foi feito:\* painel de sugestões acionáveis a partir de dados agregados do tenant (conversão de orçamentos, ocupação da agenda, inadimplência, ROI de canais CRM), com disclosure de IA. \*\*`InsightsService`\*\* (`ai/insights.service.ts`): agrega `DashboardService.getDashboard` (S42, reexporta agora `DashboardData` tipado) + `computeOccupancy` (minutos agendados / capacidade via `Availability`, mesmo padrão de contagem de ocorrência de dia-da-semana) + `computeDelinquency` (parcelas `PENDING`/`OVERDUE` vencidas / total vencendo no período) + `CrmService.reportBySource` (S44, ROI por canal) — \*\*nunca envia PII ao modelo\*\*, só números e nomes de canal (provado por teste: `userContent` não pode casar `/cpf|telefone|@|nome:/i`). Cacheado no Redis \*\*TTL 1h\*\* (chamada a LLM tem custo, TTL mais longo que o dashboard) com "to" default \*\*arredondado para a hora\*\* (sem isso, toda chamada sem período explícito — o caso comum da tela web — geraria uma chave de cache diferente a cada milissegundo, esvaziando o TTL). \*\*Disclosure de IA HARD-CODED\*\* (nunca gerado pelo modelo, §5). Prompt em `askForInsights` pede um array JSON (`type/title/explanation/suggestion`, 2 a 5 itens); `parseInsights` remove blocos ` ```json ` e falha \*\*graciosamente\*\* (array vazio, sem lançar) se a resposta não vier em JSON válido. `GET /ai/insights?from=&to=` (`billing:read`) + `POST /ai/insights/invalidate` (`billing:write`). \*\*Tela web `/insights`\*\* (Server Component): 3 cards de KPI (Conversão/Ocupação/Inadimplência), seção de ROI por canal, lista de sugestões com badge por tipo, aviso âmbar quando `aiAvailable:false` (chave ausente), botão "Atualizar" (`RefreshButton` client + `refreshInsightsAction` → invalida cache → `router.refresh()`).

  \- \*Arquivos tocados (todos já existiam como WIP; só revisados/formatados nesta sessão):\* `apps/api/src/ai/{insights.service,insights.controller,ai.module}.ts`, `apps/api/src/analytics/dashboard.service.ts` (exporta `DashboardData` tipado, reusado pelo insights), `apps/api/src/integrations/anthropic/anthropic.service.ts` (`tools` virou opcional — S50 exigia tool-use, S51 só texto), `packages/api-client/src/index.ts` (+`getInsights`/`invalidateInsights` + tipos), `apps/api/test/insights.service.spec.ts` (9 casos), `apps/web/app/insights/{page,actions,refresh-button}.tsx`. \*\*Removidos antes do commit\*\* (scripts de verificação ao vivo descartáveis da sessão anterior, sem valor de permanecer no repo — §8/§9): `apps/api/seed-s51-data.ts`, `apps/api/verify-s51.ts`, `apps/api/preload-anthropic-stub.cjs`. Sem migration (só lê dados existentes), sem dep nova.

  \- \*Decisões:\* `AnthropicService.createMessage` generalizado (`tools` opcional) para servir tanto o agente com tool-use (S50) quanto uma geração de texto simples (S51) — mesmo proxy fail-closed/endpoint-fixo, sem SSRF. Cache mais longo que o dashboard (1h vs 5min) porque uma chamada a LLM tem custo real, ao contrário de uma agregação Prisma. Ocupação/inadimplência calculadas por queries próprias (não reusam o dashboard além da conversão) porque o dashboard não expõe esses dois indicadores. Permissões reusam `billing:*` (sem re-seed). Resposta da IA tratada como \*\*não-confiável\*\*: parse falho ou insights ausentes nunca derruba o endpoint — métricas sempre voltam, `insights` só some.

  \- \*Verificação:\* `pnpm lint` (8/8)/`tsc -p tsconfig.build.json --noEmit` da api (sem cache do turbo, conforme lição da S45)/`build` (4/4, `/insights` compilou dinâmico)/`test` (277 pass, 2 skip)/`format:check` (limpo nos arquivos mantidos; `apps/web/app/central` segue com drift pré-existente da S48, não tocado)/`pnpm audit` \*\*zero vulnerabilidades\*\* — todos verdes. \*\*AO VIVO\*\* (reaproveitados os containers efêmeros `vero-s51-pg`/`vero-s51-redis` de 24h atrás, mas com \*\*processo Node MATADO E RECRIADO do build atual\*\* — a 1ª tentativa de verificação bateu sem querer num processo obsoleto de ontem ainda vivo na porta 3335, servindo dados por stub in-process; matei o PID e subi de novo do `dist/` fresco antes de validar de verdade): `GET /health` 200; login → `GET /ai/insights` sem cache → métricas reais batendo com o banco (conversão 50%, ocupação 6%, inadimplência 100%) → com `ANTHROPIC_API_KEY` inválida (stub), a chamada real à Anthropic falha (401) e o serviço \*\*degrada graciosamente\*\* (`aiAvailable:true`, `insights:[]`, sem erro 500) — comportamento correto do `catch` em `askForInsights`; `POST /ai/insights/invalidate` → `{cleared:1}`; sem token → \*\*401\*\*. \*\*Web (Playwright, processo Next também recriado do zero após matar uma instância presa na porta 3010):\* login → `/insights` renderiza os 3 KPIs, ROI por canal (Google Ads), disclosure de IA, "Sugestões geradas por IA (0)/Nenhuma sugestão gerada ainda" (degradação graciosa refletida na UI) — screenshot confirma o layout completo.\* Ambiente efêmero derrubado (containers + processos + `.env`/`.env.local` removidos); scripts Playwright descartáveis removidos; árvore de trabalho limpa (só os arquivos da S51).

  \- \*Pendências p/ próxima:\* \*\*S51 COMPLETA — fecha a lista de sessões de IA planejadas (S50+S51).\*\* Próxima no backlog: \*\*Fase 6 (S52-S57)\*\* — CI/CD com gates, infra Terraform/DigitalOcean, hardening de produção, observabilidade, submissão às lojas (Apple/Google) dos 2 apps mobile. É a fase que finalmente cobre um ambiente de produção real (hoje tudo foi validado com stacks efêmeras locais). Herdadas: nenhum link de navegação para `/insights` (mesma limitação de todas as páginas web desde a S16 — sem menu comum ainda), testar a geração de insights com uma `ANTHROPIC_API_KEY` REAL (só validado com chave stub — o caminho feliz de geração de texto por IA nunca rodou contra a API de verdade, só o fail-closed), drift de formatação em `apps/web/app/central` (S48), UI web de royalties, filtro de período em `/unidades`, retrofit de filtro por unidade nos módulos, navegação comum no web, modelos `Professional`/`Room` dedicados, extrair `@vero/mobile-shared`, device-build dos apps (usuário), `docker-compose.yml`, `start`→`dist/src/main.js`, lockout por conta, invalidar cache `rbac:perms`, `MessageLog` no banco.

\- \*\*2026-07-27 · S52 — CI/CD com gates (FECHA a S52 — INÍCIO da FASE 6)\*\*  ·  \*1ª sessão a criar o repositório remoto: o projeto nunca tinha saído do git local\*

  \- \*O que foi feito:\* pipeline que impede merge ruim. \*\*Repo remoto criado\*\* (`github.com/klebergobbi/vero`, decisão do usuário), `master`→`main` renomeado, push do histórico completo (91 commits). \*\*`.github/workflows/ci.yml`\*\* com 2 jobs: \*\*`segredos`\*\* (baixa o binário OSS do gitleaks direto do GitHub Releases — não o Marketplace Action, que exige licença paga em repo privado — e roda `gitleaks detect` no histórico completo) e \*\*`ci`\*\* (`pnpm install --frozen-lockfile` → `prisma generate` → lint → format:check → test → build → \*\*`pnpm audit --audit-level=high --prod`\*\*). \*\*`.gitleaks.toml`\*\* allowlista `.env.example` (única linha que disparava a regra genérica de API key — falso positivo por design, §1). \*\*Branch protection\*\* no `main` (`required_status_checks.strict=true` nos 2 jobs, `enforce_admins=true`, sem force-push/delete) — só foi possível \*\*depois de tornar o repo público\*\* (branch protection em repo privado exige GitHub Pro; usuário escolheu tornar público em vez de pagar).

  \- \*\*\*2 bugs reais achados pela própria CI (não existentes antes, ou mascarados no ambiente local):\*\*\* \*\*(1) overrides de `brace-expansion` quebrados\*\* — as 3 regras por faixa de versão (`@<1.1.16`, `@>=2.0.0 <2.1.2`, `@>=3.0.0 <5.0.8`) no `pnpm.overrides` da raiz colapsavam TODO consumidor de `brace-expansion` (via qualquer versão de `minimatch`) para `5.0.8`, cuja API (objeto `{expand}`) é incompatível com `minimatch@3.1.5` (usado por `@nestjs/cli`/jest — espera receber `brace-expansion` como FUNÇÃO direta) → `eslint "{src,test}/**/*.ts"` (o único script de lint do monorepo com chave `{...}`, logo o único a de fato invocar `braceExpand()`) quebrava com `TypeError: expand is not a function`. \*\*Só reproduzia no Linux da CI\*\* — no Windows local o `pnpm lint` sempre "passava" porque o turbo servia o resultado do CACHE (de uma execução anterior, de antes do override quebrar), mascarando o bug — mesma classe de problema já documentada pela S45 (\*\*"gate de build mascarado" agora também vale pro lint\*\*: rodar `turbo run lint --force` pra ter certeza). \*\*Corrigido\*\* trocando as 3 regras por overrides ESCOPADOS por dependente exato (`minimatch@3.1.5>brace-expansion`, `minimatch@9.0.9>brace-expansion`, `minimatch@10.2.5>brace-expansion`) — cada major de `minimatch` recebe a versão de `brace-expansion` da SUA própria API, patcheada dentro da própria major (não um salto pra uma major incompatível). \*Erro secundário nessa correção: pinei `1.1.12` de memória sem checar advisory — `pnpm audit` acusou que o patch real fica em `>=1.1.16`; corrigido antes de seguir.\* \*\*(2) drift de formatação pré-existente em `apps/web/app/central`\*\* (herdado da S48, documentado mas nunca corrigido porque nada rodava `format:check` como gate) — agora que o CI EXIGE format:check, virou bloqueador de verdade; corrigido com `prettier --write` (só reformatação, sem mudança de lógica, conferido no diff).

  \- \*\*\*Decisão de auditoria (`--prod`):\*\* mesmo com os overrides corrigidos, sobra 1 advisory HIGH de `brace-expansion` ("DoS via unbounded expansion length", patch só em `>=5.0.8`) que NUNCA foi backportado pra linha 1.x — logo `minimatch@3.1.5` (via `jest`/`@nestjs/cli`, só devDependency, nunca roda em produção) fica permanentemente com esse achado enquanto essas ferramentas não migrarem pra um `minimatch` mais novo (checado: `@nestjs/cli` só tem alpha além do 11.x; `jest` 30 é upgrade de major fora de escopo desta sessão). Sem caminho de exploração real (o glob é sempre first-party — nosso próprio `tsconfig`/`jest.config`, nunca input de usuário). Em vez de aceitar isso ad-hoc só numa nota de log (padrão anterior do projeto, ex. `js-yaml`), \*\*formalizei a distinção estruturalmente\*\*: `pnpm audit --audit-level=high --prod` — escopo em dependências de PRODUÇÃO (o que §4/§9 realmente protege), documentado inline no workflow.\*\*

  \- \*\*Prova do aceite (\"PR com segredo, lint quebrado ou teste falhando é bloqueado\"):\* branch de teste com um teste `expect(1).toBe(2)` de propósito + um fake `STRIPE_SECRET_KEY` no `.env.example`.\* \*\*Segredo:\* o `git push` nem chegou a criar o commit remoto — o \*\*GitHub Push Protection nativo\*\* (habilitado automaticamente ao tornar o repo público) rejeitou na hora, antes até do gitleaks da CI rodar (defesa em profundidade: 2 camadas).\* Removido o fake secret, mantido só o teste quebrado → \*\*PR #1 aberto → `Testes` falhou (exit 1) → `gh pr view` mostrou `mergeStateStatus: BLOCKED` → tentativa explícita de `gh pr merge --squash` recusada: \"the base branch policy prohibits the merge\" (bloqueia até admin, `enforce_admins=true`).\* PR fechado sem merge, branch de teste deletada (local+remoto), árvore limpa.\*

  \- \*Arquivos tocados:\* `.github/workflows/ci.yml` (novo), `.gitleaks.toml` (novo), `package.json` (raiz, overrides de `brace-expansion` reescritos), `pnpm-lock.yaml` (regenerado), `apps/web/app/central/{actions,page,scheduling-center}.tsx` (só reformatação). \*\*Repositório remoto:\* `github.com/klebergobbi/vero` (público), branch `main` protegida.\*

  \- \*Verificação:\* DoD completo local (lint com `--force`, test, build com `--force`, format:check, audit --prod) verde \*antes\* de cada push. \*\*CI real no GitHub (2 execuções):\* 1ª falhou no lint exatamente como diagnosticado (prova de que o bug era real, não hipotético); 2ª — após as correções — \*\*verde nos dois jobs\*\* (`Segredos` 8s, `Lint · Test · Build · Format · Audit` 2m27s). Branch protection + teste de bloqueio (PR #1) confirmados ao vivo contra o GitHub real, não simulado.\*

  \- \*Pendências p/ próxima:\* \*\*S52 COMPLETA.\*\* Próxima no backlog: \*\*S53 — Infra como código\*\* (Terraform: Postgres/Redis Managed + Spaces + App/Droplet na DigitalOcean, ambientes dev/staging/prod com segredos distintos — primeira sessão a exigir uma conta/billing na DigitalOcean, mesma dinâmica de decisão de custo que apareceu aqui com o GitHub Pro). Herdadas: repo agora é PÚBLICO (decisão consciente do usuário — reavaliar se algum dia fizer sentido voltar a privado + Pro), `docker-compose.yml` continua não existindo (S1, útil agora que há CI pra espelhar localmente), migrar jest/`@nestjs/cli` p/ eliminar o `minimatch@3.1.5` residual quando houver janela, nenhum link de navegação entre as páginas web, UI web de royalties, filtro de período em `/unidades`, modelos `Professional`/`Room` dedicados, extrair `@vero/mobile-shared`, device-build dos apps (usuário), `start`→`dist/src/main.js`, lockout por conta, invalidar cache `rbac:perms`, `MessageLog` no banco.

\- \*\*2026-07-27 · S53 — Infra como código na DigitalOcean via Terraform (código completo, EM ABERTO — falta conta real do usuário)\*\*  ·  \*doctl já estava autenticado nesta máquina numa conta de terceiro (felipe@f2lc.com.br) — usuário confirmou que não é dele; sessão trabalhou 100% offline de qualquer conta DO real, sem tocar nessa autenticação alheia\*

  \- \*O que foi feito:\* \*\*3 bugs de produção pré-existentes corrigidos primeiro\*\* (pré-requisitos — sem eles o deploy real nem começaria): \*\*(1)\* `dist/main.js` nunca era gerado no caminho que `package.json#start` espera (`node dist/main.js`) — `nest build` sempre emitiu em `dist/src/main.js` (bug de ~50 sessões, desde a S1, nunca notado porque dev usa `nest start --watch` e a CI só checava exit code); corrigido com `rootDir:"./src"` no `tsconfig.build.json`. Isso revelou um \*\*2º bug\*\*: `incremental:true` (herdado do tsconfig base) conflita com `deleteOutDir:true` do Nest — o `tsc`, vendo o `.tsbuildinfo` achando que nada mudou, não reemitia NADA depois que `deleteOutDir` apagava a pasta, dando build "sucesso" com `dist/` vazio em builds subsequentes (reproduzido 2x seguidas ao vivo); corrigido desligando `incremental` só no build de produção. \*\*(3)\* faltava `binaryTargets` no `generator client` do Prisma — só gerava o engine nativo (Windows), nunca o `linux-musl-openssl-3.0.x` que uma imagem `node:alpine` em produção precisa; adicionado `["native","linux-musl-openssl-3.0.x"]`.\* Essas 3 correções foram commitadas e mergeadas ANTES da infra em si (PR dedicado).

  \- \*\*Dockerfiles multi-stage\* (nunca existiam — §4 A02 já exigia "não-root, base mínima, multi-stage" desde o início do projeto):\* `apps/api/Dockerfile` e `apps/web/Dockerfile`, padrão oficial Turborepo+pnpm (`turbo prune <pkg> --docker` reduz o monorepo só ao necessário antes de instalar). `apps/web` ganhou `output:"standalone"` no `next.config.mjs` (imagem final ~307MB via trace do webpack, sem instalar `node_modules` completo). \*\*3 bugs achados DURANTE a validação real com `docker build`:\* (a) `npx turbo prune` sem versão fixa, ANTES do `pnpm install`, baixava a mais nova do registry (2.10.7, não a 2.9.16 pinada no projeto) e gerava um `pnpm-lock.yaml` podado QUEBRADO ("no entry for /@alloc/quick-lru/5.2.0") — mesma classe de bug com `npx prisma generate` (buscava a v7, com breaking changes no datasource) — ambos corrigidos fixando a versão/usando `pnpm --filter @vero/api exec` em vez de `npx` cru. (b) `@vero/types` (pacote do WORKSPACE, não do registry) nunca tinha seu `dist/` copiado pro estágio final da imagem — só existia no estágio de build; corrigido com `COPY --from=builder .../packages/types/dist`. (c) `prisma` (CLI) é devDependency — `pnpm install --prod` no estágio final não o instala, então `prisma generate` ali quebrava; corrigido com o padrão "instala completo → gera → `pnpm prune --prod`" (preserva o client já gerado, remove só as devDeps depois).\*

  \- \*\*Módulo Terraform\* `infra/modules/vero-stack`: `digitalocean_database_cluster` Postgres 16 (`engine="pg"`) + `digitalocean_database_cluster` \*\*Valkey\*\* (`engine="valkey"` — desvio documentado: a DigitalOcean descontinuou "Redis" como produto Managed novo em favor do Valkey, fork drop-in-compatible; ioredis/BullMQ não mudam de código, só o nome do engine no Terraform) + `digitalocean_spaces_bucket` PRIVADO com CORS (tudo servido por URL assinada, mesmo padrão já usado no backend desde S24/S26/S32/S36/S43 — S53 finalmente entrega a peça de infra que faltava) + `digitalocean_app` (App Platform, 2 `service` a partir dos Dockerfiles via `dockerfile_path`, health check em `/health`, env vars SECRET/GENERAL incluindo `DATABASE_URL`/`REDIS_URL` vindos direto dos outputs `private_uri` dos clusters, `API_BASE_URL` do web usando o binding nativo `$${api.PRIVATE_URL}` — tráfego servidor-a-servidor do BFF nunca sai pra internet pública) + `digitalocean_project` por ambiente (agrupa tudo, visível no console também, não só no state).\* \*\*3 root modules independentes\* `infra/environments/{dev,staging,prod}` — cada um com PRÓPRIO provider/state/`terraform.tfvars` (gitignored; só `.tfvars.example` commitado, mesmo padrão do `.env.example`), satisfazendo "ambientes separados" e "segredos distintos" literalmente (states nunca se cruzam). dev com `deploy_on_push=true`; staging/prod exigem promoção deliberada. prod com `instance_count=2` (zero-downtime de deploy, §1) e domínio customizado opcional (`api_domain`/`web_domain`, null até os domínios do §0 serem confirmados).\*\*

  \- \*Decisões:\* \*\*state LOCAL por enquanto\* (cada ambiente já isola seu próprio arquivo — migrar pra um backend remoto em Spaces só quando a equipe crescer além de 1 pessoa aplicando infra, não antes).\* Compute via \*\*App Platform\*\* (não Droplets) — totalmente declarativo, sem gerenciar SO/orquestração manual, mais alinhado a "nada criado por clique manual". `pnpm prune` do módulo do workspace resolvido com o mesmo `turbo prune --docker` já testado nos Dockerfiles.

  \- \*\*\*Decisão de conta (bloqueio real, não hipotético):\*\* o `doctl` desta máquina já estava autenticado numa conta de TERCEIRO (`felipe@f2lc.com.br`, "My Team") — perguntei ao usuário antes de tocar em qualquer coisa; ele confirmou que não é dele e que precisa criar/indicar uma conta própria. \*\*Nenhuma credencial dessa conta alheia foi usada em nenhum momento\*\* — toda a sessão trabalhou 100% offline de qualquer conta DO real (só `terraform init`/`validate`/`fmt`, que não fazem chamada autenticada à API).\*\*

  \- \*Verificação:\* \*\*Docker real\* (não simulado): `docker build` das duas imagens com sucesso; `docker run` end-to-end contra Postgres+Redis efêmeros reais numa rede Docker — migrations aplicadas (`prisma migrate deploy`), API subiu, TODAS as rotas mapeadas, BullMQ schedulers registrados, \*\*`GET /health` → `{"status":"ok","info":{"database":"up","redis":"up"}}\*\*; web serviu `/login` 200 em 337ms; ambos os containers confirmados rodando como usuário não-root (`whoami` → `vero`).\* \*\*Terraform\* (sem credenciais — não há conta ainda): `terraform init`+`validate`+`fmt -check` limpos no módulo + nos 3 ambientes.\* DoD completo do monorepo (lint --force/test/build --force/format:check/audit --prod) verde antes de cada push. CI real do GitHub verde nos 2 jobs (1 iteração extra: gitleaks pegou um 2º falso-positivo do MESMO padrão da S52, agora em `terraform.tfvars.example`, resolvido estendendo o allowlist já existente).

  \- \*Pendências p/ próxima:\* \*\*S53 EM ABERTO — código completo e mergeado, mas o checkbox `\[ ]` no §10 fica como está até rodar `terraform plan`/`apply` de verdade.\* Bloqueado em UMA coisa: o usuário criar sua PRÓPRIA conta DigitalOcean + gerar um API token (Generate New Token, escopo read+write) + um par de chaves Spaces (Spaces > Access Keys) — só isso, nada mais precisa de clique manual depois. Quando existir: preencher `infra/environments/dev/terraform.tfvars` (a partir do `.example`), `terraform init`+`plan`+`apply` no `dev` primeiro, confirmar o `API_BASE_URL` com binding `$${api.PRIVATE_URL}` resolve mesmo (não pude validar isso contra a API real — é a maior incerteza que sobra no módulo), só depois replicar p/ staging/prod.\* Herdadas: repo público (S52), `docker-compose.yml` (S1, útil agora c/ Dockerfiles prontos p/ espelhar local), UI web de royalties, filtro de período em `/unidades`, navegação comum no web, modelos `Professional`/`Room` dedicados, extrair `@vero/mobile-shared`, device-build dos apps mobile (usuário), lockout por conta, invalidar cache `rbac:perms`, `MessageLog` no banco.
