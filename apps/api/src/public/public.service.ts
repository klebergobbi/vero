...
import type { OpenSlot } from "../appointment/agenda.util";
 ...
 * a agenda interna — só devolve SLOTS LIVRES (via SlotService). O booking
 * re-valida o slot e reusa o AppointmentService (conflito race-safe → 409).
 */
@Injectable()
export class PublicService {
  ...
  async listOpenSlots(
    ...
    const tenant = await this.resolveTenant(slug);
    return this.slots.openSlots(tenant.id, unitId, professionalId, dateYmd);
  }
